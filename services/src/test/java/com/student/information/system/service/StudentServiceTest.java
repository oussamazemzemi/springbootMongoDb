...
public class StudentServiceTest {
	...
	public void setup() {
		...
		Mockito.when(studentRepository.findByStudentNumber(ragcrixStudentNumber)).thenReturn(ragcrix);

		Mockito.when(studentRepository.findByEmail(yigitEmail)).thenReturn(yigit);

		Mockito.when(studentRepository.findAllByOrderByGpaDesc()).thenReturn(students.stream()
	...
	public void testDeleteStudentById() {
		...
		Mockito.verify(studentRepository, Mockito.times(1)).deleteById(ragcrix.getId());
	}
