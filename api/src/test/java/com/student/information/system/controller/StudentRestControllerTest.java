...
public class StudentRestControllerTest {
	...
	public void givenStudents_whenGetAllStudents_thenReturnJsonArray() throws Exception {
		...
		mvc.perform(get("/students/").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$", hasSize(1))).andExpect(jsonPath("$[0].name", is(ragcrix.getName())));
	}
	...
	public void givenStudent_whenFindAllByOrderByGpaDesc_thenReturnJsonArray() throws Exception {
		...
		mvc.perform(get("/students/orderByGpa/").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$", hasSize(2))).andExpect(jsonPath("$[0].name", is(yigit.getName())));
	}
	...
	public void saveStudent_itShouldReturnStatusOk() throws Exception {
		...
		mvc.perform(post("/students/save/").contentType(MediaType.APPLICATION_JSON).content(jsonString))
